...
-- ---- H (Newhouse) ----
delete from banque_provinces where banque_tag = 'H' and province in ('Alanbie','Jance','Vénéz','Alonse','Hors-Grande-Vénésie','Côte d''Avoigne','Côte-Anglaise');
...
-- ---- L (Lataylon) : Brien part (rejoint l'union H), Grande-Capitale fusionne avec Marcio ----
delete from banque_provinces where banque_tag = 'L' and province = 'Brien';
do $$
declare v_pop_gc numeric;
begin
  select population into v_pop_gc from banque_provinces where banque_tag = 'L' and province = 'Grande-Capitale';
  if v_pop_gc is not null then
    ...
    delete from banque_provinces where banque_tag = 'L' and province = 'Grande-Capitale';
  end if;
end $$;

-- ---- M (Maréchal) : Isulae+Balques -> Union des Balques, Pruxe-Slamique -> Pruxe ----
delete from banque_provinces where banque_tag = 'M' and province in ('Isulae','Balques');
insert into banque_provinces (banque_tag, province, population, part_pourcentage) values
  ('M', 'Union des Balques', 1230000, 0)
on conflict (banque_tag, province) do update set population = excluded.population;
update banque_provinces set province = 'Pruxe' where banque_tag = 'M' and province = 'Pruxe-Slamique';

-- ---- P (Fleury) : Milela-Cale+North Milela -> Union de Milela, Grâdes+Tivainne -> Union de Grâdes-Tivainne, Gibaltage -> Union de Gibaltage ----
delete from banque_provinces where banque_tag = 'P' and province in ('Milela-Cale','North Milela','Grâdes','Tivainne');
insert into banque_provinces (banque_tag, province, population, part_pourcentage) values
  ('P', 'Union de Milela', 1194951, 0),
  ('P', 'Union de Grâdes-Tivainne', 987000, 0)
on conflict (banque_tag, province) do update set population = excluded.population;
update banque_provinces set province = 'Union de Gibaltage', population = population + 0 where banque_tag = 'P' and province = 'Gibaltage';

-- Recalcule les pourcentages de TOUTES les provinces des banques touchées
-- (H, L, M, P) pour qu'ils resomment à 100 % après les fusions.
...
-- ============================================================
-- 2) FUSION DES PROVINCES — province_residence_banque (banque payeuse)
-- ============================================================
delete from province_residence_banque where province in
  ('Alanbie','Jance','Brien','North Milela','Milela-Cale','Vénéz','Alonse',
   'Hors-Grande-Vénésie','Côte d''Avoigne','Côte-Anglaise','Isulae','Balques',
   'Grâdes','Tivainne','Pruxe-Slamique','Gibaltage');
...
-- Migre les citoyens déjà inscrits sous l'ancien nom de province vers la
-- nouvelle union, pour ne pas perdre leur banque payeuse de salaire.
update citoyens set province_residence = 'Union d''Alanbie-Brien-Jance' where province_residence in ('Alanbie','Brien','Jance');
update citoyens set province_residence = 'Union de Milela' where province_residence in ('North Milela','Milela-Cale');
update citoyens set province_residence = 'Union Vénézienne' where province_residence in ('Vénéz','Alonse');
update citoyens set province_residence = 'Union de la Côte-Anglaise' where province_residence in ('Hors-Grande-Vénésie','Côte d''Avoigne','Côte-Anglaise');
update citoyens set province_residence = 'Union des Balques' where province_residence in ('Isulae','Balques');
update citoyens set province_residence = 'Union de Grâdes-Tivainne' where province_residence in ('Grâdes','Tivainne');
update citoyens set province_residence = 'Pruxe' where province_residence = 'Pruxe-Slamique';
update citoyens set province_residence = 'Union de Gibaltage' where province_residence = 'Gibaltage';


-- ============================================================
-- 3) CORRECTION : "trésorerie provinciale" -> trésorerie des BANQUES
--    NATIONALES (les banques n'ont qu'une seule trésorerie chacune).
-- ============================================================

-- Retrait de l'ancien (mauvais) système de trésoreries provinciales.
drop trigger if exists trg_renflouer_provinciale on tresoreries_provinciales;
drop function if exists gouv_transferer_vers_provinciale(text, numeric, text);
drop function if exists gouv_transferer_depuis_provinciale(text, numeric, text);
drop function if exists gouv_liste_tresoreries_provinciales();
drop table if exists tresoreries_provinciales;

-- Renfloue automatiquement une banque nationale à 50 000 R$ dès que sa
-- trésorerie atteint 0 (ou moins) — même si ça met une trésorerie du
-- gouvernement en dette.
...
drop trigger if exists trg_renflouer_banque on banques_nationales;
create trigger trg_renflouer_banque before insert or update on banques_nationales
  for each row execute function _renflouer_banque_nationale();
...
-- gouv_payer_civil() pige maintenant dans la trésorerie de la BANQUE
-- NATIONALE de la province (via province_residence_banque), pas dans
-- une trésorerie provinciale séparée.
...
-- Vue pratique pour l'admin : trésorerie de chaque banque nationale.
create or replace function gouv_liste_tresoreries_banques()
returns table(tag char(1), nom text, tresorerie numeric) language sql stable security definer set search_path = public as $$
  select tag, nom, tresorerie from banques_nationales order by tag;
$$;
grant execute on function gouv_liste_tresoreries_banques() to authenticated;
